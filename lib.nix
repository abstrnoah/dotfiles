...
rec {
  ...
  curry = key: f: # function of the form (set@{ key, ... }: ...)
    value:
    set@{ ... }:
  ...
  make_amendable = f:
    s@{ ... }:
    (f s) // {
      __constructor = make_amendable f;
      __preimage = s;
    };
  amend = amender: # new: old: final
  ...
  make_env = { name ? "env", deps, paths ? [ "/" ], ... }:
    assert nixpkgs != null;
    let
      # TODO FIXME awful terrible doesn't work when two deps have same common dep
      nixphile_hook_pre = bundle_scripts (map (get "nixphile_hook_pre")
  ...
  make_source = { source # NOTE: should NOT be a store path
    , excludes ? [ ], ... }:
